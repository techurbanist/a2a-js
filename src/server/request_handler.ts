...
export class DefaultA2ARequestHandler implements A2ARequestHandler {
   ...
   */
  private _appendMessageToTask(messageSendParams: MessageSendParams, task?: Task): void {
    if (task) {
   ...
   * @returns Promise resolving to the send message response
   */
  async onMessageSend(request: SendMessageRequest): Promise<SendMessageResponse> {
    const messageSendParams = request.params;
